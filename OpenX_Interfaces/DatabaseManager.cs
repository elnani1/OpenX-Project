...
namespace OpenX_Interfaces
{
    ...
    {
        private string connectionString = "Data Source=DESKTOP-NP92TUR\\ALEX1;Initial Catalog=OpenX;Integrated Security=True";
        ...
        {
            String sql = "insert into ReglasDeAcceso (UsuarioAsignado, DispositivoAsignado, Horario) Values (@usurio, @dispositivo, @horario)";
            using (SqlConnection conexion = new SqlConnection(connectionString))
            {
                using (SqlCommand comando = new SqlCommand(sql, conexion))
                {
